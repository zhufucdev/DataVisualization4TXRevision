...
import { Table, ShowAverage, Partial, ShowMaximun, ShowTrend } from "./formutils";
...
}> = ({ source, reference, width,
  height, framesAwait, translation, translationDuration, easing,
  dark, primaryColor, label }) => {
    const [data, maxData, minData] = extractData(source, reference || 'time');

    const frame = useCurrentFrame();
    framesAwait = framesAwait || 0;
    const defaultEasing = Easing.inOut(Easing.exp);
    const drawingProgress = interpolate(
      frame,
      [framesAwait, framesAwait + (translation ? (translationDuration || 50) : 90)],
      [0, 1],
      {
    ...
    function getAuxiliaryLines(): ReactElement | null {
      ...
      function getTrendIndicators(effect: ShowTrend): ReactElement {
        const section = effect.section, width = section.to - section.from;
        const set = data[section.column];

        const x = width * drawingProgress + section.from;
        const left = Math.floor(x), right = Math.floor(x) + 1;
        const target = x < section.to
          ? {
            index: x,
            value: interpolate(
              drawingProgress,
              [(left - section.from) / width, (right - section.from) / width],
        ...
        const ver: React.CSSProperties = {
          ...
          ...commonStyle,
          bottom: 0,
          left: `${100 * target.index / xLength}%`,
          width: auxiliaryLineStroke,
          height: `${100 * (target.value - minData) / yLength}%`
        }
        const hor: React.CSSProperties = {
          ...dottedLine(true),
          ...commonStyle,
          bottom: `${100 * (target.value - minData) / yLength}%`,
          left: 0,
          width: `${100 * target.index / xLength}%`,
          height: auxiliaryLineStroke
        }
        const label: React.CSSProperties = {
          ...surface(true),
        ...
        return <>
          <div style={ver} />
          <div style={hor} />
          <span style={label}>{labelStr(effect, target.value)}</span>
        </>
      }
      ...
      } else if (source.visualEffect instanceof ShowTrend) {
        return getTrendIndicators(source.visualEffect);
      } else {
        return null;
      }
    }
